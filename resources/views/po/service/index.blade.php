                ...
                <div class="card-header pb-0 d-flex justify-content-between align-items-center">
                    <div class="d-flex">
                        ...
                        <button type="button" class="btn btn-dark btn-2x me-2" data-bs-toggle="modal" data-bs-target="#exampleModal">
                            <i class="fas fa-plus"></i> Tambah PO Service
                        </button>
                        <!-- Modal input data -->
                            ...
                            <div class="modal-dialog modal-dialog-centered modal-lg">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h5 class="modal-title" id="exampleModalLabel">Tambah PO Service</h5>
                                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                    </div>
                                    <div class="modal-body" style="max-height: 450px; overflow-y: auto;">
                                        <!-- Isi formulir di sini -->
                                        <form action="{{ route('poservice.store') }}" method="POST">
                                            @csrf
                                            <!-- Isi formulir dengan input yang sesuai -->
                                            <div class="mb-3">
                                                <label for="idServicePO" class="form-label">Nomor PO Service</label>
                                                <input type="number" class="form-control" id="idServicePO" name="idServicePO">
                                            </div>
                                            <div class="mb-3">
                                                ...
                                                <select class="form-select" id="idServicePR" name="idServicePR">
                                                    ...
                                                    <option value="{{ $prservice->idServicePR}}">
                        ...
                        <!-- Reload Data Terbaru-->
                        <a href="{{ route('poservice') }}" class="btn btn-light btn-2x me-2">
                ...
                <div class="card-body px-3 pt-0 pb-2">
                    <div class="table-responsive p-0">
                        <table class="table align-items-center mb-0">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td style="font-size: 14px;">{{ $poservices->idServicePO }}</td>
                                    <td style="font-size: 14px;">{{ $poservices->idServicePR }}</td>
                                    <td style="font-size: 14px;">{{ $poservices->judulPekerjaan }}</td>
                                    <td style="font-size: 14px;">
                                        <a href="#" class="btn btn-sm btn-outline-warning editButton" data-bs-toggle="modal" data-bs-target=".modalEdit" data-id="{{ $poservices->idServicePO }}">Edit</a>
                                        <form action="{{ route('poservice.destroy', $poservices ->idServicePO) }}" method="POST" class="d-inline deleteForm" data-id="{{ $poservices->idServicePO }}">
                                            @csrf
                                            @method('DELETE')
                                ...
                                <div class="modal fade modalEdit" tabindex="-1" aria-labelledby="modalEditLabel" aria-hidden="true">
                                    <div class="modal-dialog modal-dialog-centered modal-lg">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h5 class="modal-title" id="modalEditLabel">Edit PR Reimburst</h5>
                                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                            </div>
                                            <div class="modal-body " style="max-height: 450px; overflow-y: auto;">
                                                <!-- Form untuk mengedit prservice -->
                                                <form action="{{ route('poservice.edit', $poservices->idServicePO ) }}" method="POST" id="editForm">
                                                    @csrf
                                                    @method('PUT')
                                                    <!-- Isi form sesuai kebutuhan -->
                                                    <div class="mb-3">
                                                        <label for="idServicePO " class="form-label">Nomor PO Reimburst</label>
                                                        <input type="number" class="form-control" id="idServicePO" name="idServicePO" value="{{ $poservices->idServicePO  }}">
                                                    </div>
                                                    <div class="mb-3">
                                                        <label for="idServicePR " class="form-label">Nomor PR Reimburst</label>
                                                        <input type="number" class="form-control" id="idServicePR" name="idServicePR" value="{{ $poservices->idServicePR  }}">
                                                    </div>
                                                    <div class="mb-3">
                                                        <label for="judulPekerjaan" class="form-label">Nama Pekerja</label>
                                                        <input type="text" class="form-control" id="judulPekerjaan" name="judulPekerjaan" value="{{ $poservices->judulPekerjaan }}">
                                                    </div>
                        ...
                        <div class="float-start mx-2">
                            <p class="text-muted">
                                Showing {{ $poservices->firstItem() }} to {{ $poservices->lastItem() }} of {{ $poservices->total() }} entries
                            </p>
                        </div>
                        <div class="clearfix"></div>
                        <nav aria-label="Page navigation example">
                            <ul class="pagination">
                                <li class="page-item {{ ($poservices->onFirstPage()) ? 'disabled' : '' }}">
                                    <a class="page-link" href="{{ $poservices->url(1) }}" aria-label="First">
                                        <span aria-hidden="true">&laquo;&laquo;</span>
                                    </a>
                                </li>
                                <li class="page-item {{ ($poservices->onFirstPage()) ? 'disabled' : '' }}">
                                    <a class="page-link" href="{{ $poservices->previousPageUrl() }}" aria-label="Previous">
                                        <span aria-hidden="true">&laquo;</span>
                                    </a>
                                </li>
                                @for ($i = max(1, $poservices->currentPage() - 2); $i <= min($poservices->lastPage(), $poservices->currentPage() + 2); $i++)
                                    <li class="page-item {{ ($poservices->currentPage() == $i) ? 'active' : '' }}">
                                        <a class="page-link" href="{{ $poservices->url($i) }}">{{ $i }}</a>
                                    </li>
                                    @endfor
                                    <li class="page-item {{ ($poservices->hasMorePages()) ? '' : 'disabled' }}">
                                        <a class="page-link" href="{{ $poservices->nextPageUrl() }}" aria-label="Next">
                                            <span aria-hidden="true">&raquo;</span>
                                        </a>
                                    </li>
                                    <li class="page-item {{ ($poservices->currentPage() == $poservices->lastPage()) ? 'disabled' : '' }}">
                                        <a class="page-link" href="{{ $poservices->url($poservices->lastPage()) }}" aria-label="Last">
